...
class Tracker:
    ...
    def assign_ids(self, pred, image): # {'Car': [{'box2d': [x1, y1, x2, y2], 'score': s}], 'Pedestrian': [{'box2d': [x1, y1, x2, y2], 'score': s}]}
        ...
        # image = adjust_brightness(image, 0.1)
        ...
        for cls, boxes in pred.items():
            ...
            for p in last_preds:
                ...
                if p['occlusion']==0 and area_inside>1024*1.2 and size_flag and False:
                    ew = w*0.6 if cls=='Car' else w*0.4
                    eh = h*0.6 if cls=='Car' else h*0.4
